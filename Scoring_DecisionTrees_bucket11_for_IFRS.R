...
df <- read.csv("buckets\\Bucket11.csv")
nrow(df)
...
pd_dt_bucket11 <- ctree(default_next ~ max_DPD_upto + ratio_passed_installments + last_payment + payments_per_installments + 
                                            ownership + education + age + gender + household_children + on_address +
                                            experience_employer + marital_status +
                                            max_delay + credits_cum + days_diff_last_credit,
                             data = df80, 
                             controls = ctree_control(mincriterion = 0.99, minsplit = 250))
plot(pd_dt_bucket11)

# Get stats
df80$cluster <- where(pd_dt_bucket11)
...
score_train <- predict(pd_dt_bucket11, df80)
score_test <- predict(pd_dt_bucket11, df20)
...
score_all <- predict(pd_dt_bucket11, df)
df <- cbind(df,score_all)
names(df)[ncol(df)] <- "default_predict"
max(score_all)
min(score_all)
mean(score_all)
nrow(df)
...
splitted_names(pd_dt_bucket11)

# Output model 
setwd("C:\\Projects\\PD_model\\models\\")
save(pd_dt_bucket11, file = "PD_Bucket11.RData")
