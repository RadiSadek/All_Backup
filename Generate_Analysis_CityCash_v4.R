...
df_final$Total_Paid <- ifelse(is.na(df_final$amount_paid),0,df_final$amount_paid)
...
bins <- c(0,0.1,0.13,0.15,0.17,0.19,0.21,0.22,0.25,0.26,0.28,0.3,0.32,
          0.34,0.37,0.39,0.43,0.46,0.5,0.525,0.55,0.575,0.6,0.8)
bins <- c(0,0.1,0.13,0.15,0.17,0.19,0.21,0.22,0.25,0.26,0.28,0.3,0.32,
          0.34,0.37,0.39,0.43,0.46,0.5,0.525,0.55,0.575,0.6,0.8)
...
df_analysis <- merge(sum_principal_bads ,sum_principal_goods, 
...
df_analysis <- cbind(df_analysis,
    t(table(df_final$default_flag,df_final$Score_Group))[,1])
df_analysis <- cbind(df_analysis,
    t(table(df_final$default_flag,df_final$Score_Group))[,2])
...
for (i in (1:nrow(df_analysis))) {
  df_analysis$Bads_Rate[i] <- df_analysis$True_Bads[i]/
    (df_analysis$True_Goods[i]+df_analysis$True_Bads[i])
}
...
# DISPLAY RESULT 
View(df_analysis)
write.xlsx(df_analysis,"Profit_Analysis.xlsx")
