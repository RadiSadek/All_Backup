...
df <- subset(df,df$product_cat=="Credirect_Flex")
...
df$age_cut <- ifelse(df$age<=20,"20_less",
     ifelse(df$age<=23,"21_23","more_24"))
df$age <- as.factor(df$age_cut)
df <- within(df, age <- relevel(age, ref = "more_24"))
...
df$marital_status_cut <- ifelse(is.na(df$marital_status), "other",
    ifelse(df$marital_status %in% c(4,5),"4_5",
    ifelse(df$marital_status %in% c(1,3),"1_3",df$marital_status)))        
...
df$household_total_cut <- ifelse(is.na(df$household_total), "NA",
 ifelse(df$household_total==1,"1",
 ifelse(df$household_total==2,"2","more_3")))
...
df$experience_employer_cut <- ifelse(is.na(df$experience_employer), "0_12",
...
df$status_active_total_cut <- ifelse(is.na(df$status_active_total),
      "other",
  ifelse(df$status_active_total %in% c(-1), "no_credit",
  ifelse(df$status_active_total %in% c(0,71,72),"other",
  ifelse(df$status_active_total %in% c(73,74,75),"73_74_75","other"))))
...
df$source_entity_count_total_cut <- ifelse(is.na(df$source_entity_count_total),
  "other",
  ifelse(df$source_entity_count_total==0,"0_and_more_5",
  ifelse(df$source_entity_count_total>=5,"0_and_more_5","other")))
...
df$seon_var_cut <- ifelse(is.na(df$seon_var), "1_7",
    ifelse(df$seon_var %in% c(0),"0",
    ifelse(df$seon_var %in% c(1:7),"1_7","more_8")))     
df$seon_var <- as.factor(df$seon_var_cut)
df <- within(df, seon_var <- relevel(seon_var,ref = "1_7"))
gb_index_fct(df, df$seon_var, "default_flag")

# Fraud score manual 2 : YES
gb_index_fct(df, df$seon_var2, "default_flag")
plot_infotables(IV, "seon_var2")
df$seon_var2_cut <- ifelse(is.na(df$seon_var2), "more_2",
    ifelse(df$seon_var2 %in% c(0,1),"less_1","more_2"))
df$seon_var2 <- as.factor(df$seon_var2_cut)
df <- within(df, seon_var2 <- relevel(seon_var2,ref = "more_2"))
gb_index_fct(df, df$seon_var2, "default_flag")

# API_period : OK 
plot_infotables(IV, "API_period")
df$API_period_cut <- 
  ifelse(is.na(df$API_period),"more_8",
  ifelse(df$API_period<=7,"less_7","more_8"))
df$API_period <- as.factor(df$API_period_cut)
df <- within(df, API_period <- relevel(API_period, ref = "more_8"))
gb_index_fct(df, df$API_period, "default_flag")

# API_amount : OK 
plot_infotables(IV, "API_amount")
df$API_amount_cut <- 
  ifelse(is.na(df$API_amount),"250_650",
  ifelse(df$API_amount<=150,"less_150",
  ifelse(df$API_amount<=200,"200",
  ifelse(df$API_amount<=650,"250_650","more_700"))))    
df$API_amount <- as.factor(df$API_amount_cut)
df <- within(df, API_amount <- relevel(API_amount, ref = "250_650"))
...
# API device : YES
gb_index_fct(df, df$API_device, "default_flag")
df$API_device_cut <- ifelse(is.na(df$API_device), "other",
   ifelse(df$API_device=="Windows", "Windows", "other"))
df$API_device <- as.factor(df$API_device_cut)
df <- within(df,API_device <- relevel(API_device,ref = "other"))
gb_index_fct(df, df$API_device, "default_flag")

# API Email char : YES
plot_infotables(IV, "API_email_char_ratio")
df$API_email_char_ratio_cut <- ifelse(is.na(df$API_email_char_ratio), "other",
  ifelse(df$API_email_char_ratio<=0.6,"0_0.6","other"))
df$API_email_ratio_char <- as.factor(df$API_email_char_ratio_cut)
df <- within(df,API_email_ratio_char <- relevel(API_email_ratio_char,
                                                ref = "other"))
gb_index_fct(df, df$API_email_ratio_char, "default_flag")

# API Referral Source : YES
gb_index_fct(df, df$API_referral_source, "default_flag")
df$API_referral_source_cut <- ifelse(is.na(df$API_referral_source), "other",
    ifelse(df$API_referral_source=="facebook","facebook","other"))
df$API_referral_source <- as.factor(df$API_referral_source_cut)
df <- within(df,API_referral_source <- relevel(API_referral_source,
                                                ref = "other"))
gb_index_fct(df, df$API_referral_source, "default_flag")
...
list_var_model <-  c("default_flag",
   "age","gender","ownership","education","status_work","experience_employer",
   "purpose",
   "cred_count_total",
...
coefficients <- as.data.frame(
  cbind(names(df_Log_Credirect_App_payday$coefficients),
  df_Log_Credirect_App_payday$coefficients))
write.xlsx(coefficients,"results\\Coefficients_PayDay.xlsx")
...
##########################################
bins <- c(0,0.025,0.0375,0.05,0.0625,0.07,0.075,0.08,
...
##############################
score_all <- predict(df_Log_Credirect_App_payday, newdata=df, 
                     type="response")
names(df)[length(df)] <- 'Score'
df$default_flag_predict <- ifelse(df$Score>0.3,1,0)
write.csv(df, "data\\Results_Logit_for_Analysis_PayDay.csv")
...
# Save RData   #
################
save(df_Log_Credirect_App_payday, 
 file = "rdata\\Credirect_new_PayDay.rdata")
...
df_new <- df
score_all <- predict(df_Log_Credirect_App_payday, 
