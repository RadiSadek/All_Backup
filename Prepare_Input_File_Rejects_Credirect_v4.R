...
# Subset some more
df <- subset(df,df$status %in% c(2))
df <- subset(df,df$online_offline=="online")
df <- subset(df,df$sub_status==120)
df <- subset(df,df$has_prev_credits==0)

# Correct some fields
df$month <- substring(df$date_entry, 1, 7)
...
# Filter time window
df <- subset(df, df$date_entry<"2020-09-30" & df$date_entry>"2018-04-10")
...
seon_mails <- read.xlsx(paste("C:\\Projects\\Application_Scoring\\",
  "Credirect_v4\\data\\SEON_after_assessment_FINAL.xlsx",sep=""))
...
phone_numbers <- read.xlsx(paste(dir_old_phone,
...
df <- subset(df,df$date_entry>="2019-07-01")

# Select output columns
all_cols <- c(
  
  "egn","id","client_id","date","default_flag","product_cat",
  "amount_paid","amount",
...
# Output results\
write.csv(df,"input_current_rejects.csv")
