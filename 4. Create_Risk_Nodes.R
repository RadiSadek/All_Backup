...
node_def$defaults_citycash_agg <- NA
node_def$counts_citycash_agg <- NA
node_def$citycash_agg <- NA
for(i in 1:nrow(node_def)){
  ...
  node_def$citycash_agg[i] <- nrow(here)
  node_def$defaults_citycash_agg[i] <- sum(here$defaults_citycash)
  node_def$counts_citycash_agg[i] <- sum(here$counts_citycash)
}

node_def$defaults_citycash <- node_def$defaults_citycash_agg
node_def$counts_citycash <- node_def$counts_citycash_agg
node_def$def_pct_citycash <- round(node_def$defaults_citycash / 
                                   node_def$counts_citycash,4)*100
...
node_def <- node_def[,c(11,12,8,10)]
 
# Create subsets
node_def_citycash <- subset(node_def,node_def$counts_citycash>=min_counts)

# Remove certain obsolete nodes
node_def_citycash <- subset(node_def_citycash,
  !(node_def_citycash$node_lat==42.6925 & node_def_citycash$node_lon==23.32))
node_def_citycash <- subset(node_def_citycash,
  !(node_def_citycash$node_lat==42.4175 & node_def_citycash$node_lon==25.6125))


# Output results for GIS visualization 
write.csv(node_def_citycash,paste("C:\\Projects\\Geo_Segmentation\\result\\",
  "Defaults_per_Node_CityCash.csv",sep=""
))


# Output nodes with Default Rate >= 65%
write.xlsx(node_def_citycash[
  node_def_citycash$def_pct_citycash>=minimal_default_rate_pct,],
