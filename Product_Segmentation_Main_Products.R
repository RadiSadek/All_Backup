...
df <- subset(df , df$product!="Пенсионер" & df$product!="Друг" & df$product!="BigFin")

# Analyze 0% of all credirect ==> no result
#df <- subset(df, df$product_id %in% c(9,48,25,36,41,43,50,28,26,37,42,44,49,27))
#df$product <-  ifelse(df$product_id %in% c(41,43,50,28,42,44,49,27) , "0%","Other")
