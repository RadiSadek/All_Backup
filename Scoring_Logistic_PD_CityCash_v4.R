...
# Load data #
setwd("C:\\Projects\\Application_Scoring\\CityCash_v4\\")
df <- read.csv("data\\input_v2.csv", sep=",")
df_raw <- df
...
list_all_var_model <-  c("credit_number","default_flag", "date",
  "amount_paid", "amount",
  "flag_location_curr","flag_location_offi","same_city",
  "distance_curr_offi","self_approval",
  ...
  "maturity",
  "status_finished_total", "new_status_finished_total","cession_bin",
  "status_active_total",
...
df <- within(df, same_city <- relevel(same_city, ref = "1"))
...
df$purpose_cut <- ifelse(is.na(df$purpose), "other",
  ifelse(df$purpose %in% c(3,6),"3_6","other"))
...
df <- within(df, ratio_installment_income <- 
               relevel(ratio_installment_income, ref = "0.05_0.11"))
...
# NEW Status finished total : YES 
gb_index_fct(df,df$new_status_finished_total,"default_flag")
df$new_status_finished_total_cut <- 
  ifelse(is.na(df$new_status_finished_total),"other",
  ifelse(df$new_status_finished_total %in% c(1,711,721,731,741,751),"cession",
  ifelse(df$new_status_finished_total %in% c(71,72,73,74,75),"no_cession",
  ifelse(df$new_status_finished_total %in% c(0),"other",
      "other"))))
df$new_status_finished_total <- as.factor(df$new_status_finished_total_cut)
df <- within(df, new_status_finished_total <- relevel(new_status_finished_total, 
      ref = "other"))
gb_index_fct(df, df$new_status_finished_total, "default_flag")

# Cession : YES 
gb_index_fct(df,df$cession_bin,"default_flag")
df$cession_bin_cut <- 
  ifelse(df$cession_bin=="cession","cession","other")
df$cession_bin <- as.factor(df$cession_bin_cut)
df <- within(df, cession_bin <- relevel(cession_bin, 
   ref = "other"))
gb_index_fct(df, df$cession_bin, "default_flag")
...
df$source_entity_count_total_cut <- 
   ifelse(is.na(df$source_entity_count_total), "5_less",
   ifelse(df$source_entity_count_total %in% c(0:5), "5_less","6_more"))
df$source_entity_count_total <- as.factor(df$source_entity_count_total_cut)
df <- within(df, source_entity_count_total <- relevel(
  source_entity_count_total,ref = "5_less"))
...
df$cred_count_total_cut <- 
  ifelse(is.na(df$cred_count_total), "6_less",
  ifelse(df$cred_count_total %in% c(0:6), "6_less","7_more"))
df$cred_count_total <- as.factor(
  df$cred_count_total_cut)
df <- within(df, cred_count_total <- relevel(
  cred_count_total,ref = "6_less"))
...
df$outs_overdue_ratio_total_cut <- ifelse(
  is.na(df$outs_overdue_ratio_total), "0.1_0.5",
   ifelse(df$outs_overdue_ratio_total==-999,"0.1_0.5",
   ifelse(df$outs_overdue_ratio_total==0,"0",
   ifelse(df$outs_overdue_ratio_total<=0.1,"0_0.1",
   ifelse(df$outs_overdue_ratio_total<=0.5,"0.1_0.5","more_0.5")))))
df$outs_overdue_ratio_total <- as.factor(df$outs_overdue_ratio_total_cut)
df <- within(df, outs_overdue_ratio_total <- relevel(
  outs_overdue_ratio_total, ref = "0.1_0.5"))
...
df$flag_location_curr_cut <- ifelse(
  is.na(df$flag_location_curr), "other",
  ifelse(df$flag_location_curr==1,"1","other"))
df$flag_location_curr <- as.factor(df$flag_location_curr_cut)
df <- within(df, flag_location_curr <- relevel(
  flag_location_curr, ref = "other"))
gb_index_fct(df, df$flag_location_curr, "default_flag")
...
# Distance between current and official address : YES 
plot_infotables(IV, "distance_curr_offi")
df$distance_curr_offi_cut <- 
  ifelse(is.na(df$distance_curr_offi), "other",
  ifelse(df$distance_curr_offi<=1.5,"0_1.5","other"))
df$distance_curr_offi <- as.factor(df$distance_curr_offi_cut)
df <- within(df, distance_curr_offi <- relevel(
  distance_curr_offi, ref = "other"))
gb_index_fct(df, df$distance_curr_offi, "default_flag")

# Self approval : YES 
gb_index_fct(df, df$self_approval, "default_flag")
...
df <- within(df, self_approval <- relevel(
  self_approval, ref = "other"))
gb_index_fct(df, df$self_approval, "default_flag")
...
list_var_model <-  c("default_flag",
 ...
 "education", "status_work",
 "maturity","ratio_installment_income",
 "experience_employer",
 "on_address",
 #"new_status_finished_total",
 #"cession_bin",
 #"cred_count_total",
...
# Save RData #
##############
save(df_Log_CityCash_App, 
     file = "rdata\\PD_Logistic_Application_CityCash.rdata")
...
##########################################
quantile(df$score, (0:20)/20)
bins <- c(0,0.1,0.13,0.15,0.17,0.19,0.21,0.22,0.25,0.26,0.28,0.3,0.32,
          0.34,0.37,0.39,0.43,0.46,0.52,0.6,0.8)
...
write.csv(df_anal,"data\\Results_Logit_for_Analysis.csv")
