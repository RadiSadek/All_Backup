...
df$return_rate60 <- ifelse(df$return_rate60>1, 1, ifelse(df$return_rate60<0,0,df$return_rate60))
df$return_rate60 <- ifelse(df$return_rate60>0.2, 1, 0)
df <- subset(df,df$EAD60>10)
...
list_var_model <-  c("return_rate","gender", "flag_paid_installments","last_payment_before_def","ratio_last_amount_paid",
                     ...
                     "payment_time_diff60", "maturity","total_income","ratio_paid_installments", "ratio_passed_installments",
                     "refinance_previous","default_previous","default_cum","refinance_cum","return_rate60","paid_time60","return_rate60")
...
split <- sample.split(df_select$return_rate60, SplitRatio = 0.8)
df80 <- subset(df_select, split == TRUE)
df20 <- subset(df_select, split == FALSE)


# Build decision tree model from "party" library
decision_tree_model <- ctree(return_rate60 ~ education + ratio_passed_installments + ratio_last_amount_paid + 
                             max_delay + gender + age + ratio_installment_income + last_payment_before_def + 
                             days_diff_last_credit + experience_employer + period + paid_time60 + 
                             credits_cum, 
                             data = df80, 
...
ROCRpred <- ROCR::prediction (score_train, df80$return_rate60)
...
ROCRpred <- ROCR::prediction (score_test, df20$return_rate60)
ROCRperf <- performance (ROCRpred, "tpr", "fpr")
plot(ROCRperf,col='blue', lwd=5)
par(new=TRUE)
plot(y, col='green',axes=F,xlab='',ylab='',lwd=1)
...
df_ks <- data.frame(t(table(df_train$return_rate60,df_train$Score))[,1])
names(df_ks)[1] <- "True_Goods_Train"
df_ks$True_Bads_Train <-t(table(df_train$return_rate60,df_train$Score))[,2]
df_ks$True_Goods_Test <- t(table(df_test$return_rate60,df_test$Score))[,1]
df_ks$True_Bads_Test <- t(table(df_test$return_rate60,df_test$Score))[,2]
...
decision_tree_model_60 <- decision_tree_model
setwd("C:\\Projects\\LGD_model\\models\\20pct\\")
save(decision_tree_model_60, file = "Decision_Trees_60_20pct.rdata")
