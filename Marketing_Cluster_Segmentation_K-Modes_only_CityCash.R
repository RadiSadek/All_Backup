...
kmodel <- kmodes(df, modes=6, iter.max = 1000, weighted = FALSE, fast = TRUE)
...
character <- as.data.frame(lapply(names(df[ , !unlist(lapply(df, is.numeric))]),
                                  mfv_cat))
...
# Check results on most frequent value on cluster 1 
# (pay attention: it is different than clustering)
cluster1 <- subset(df, df$cluster==1)
cluster1$id <- 1 
c1 <- setDT(cluster1)[, .N, by=.(id,age_range,gender,nb_children_range,
    marital_status,wnership,address_cat,
    total_income_range,education)][, .SD[which.max(N)], by = id]
