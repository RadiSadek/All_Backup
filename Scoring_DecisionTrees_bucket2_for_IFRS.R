...
df <- read.csv("buckets\\1st_method\\Bucket2.csv")
nrow(df)
...
pd_dt_bucket2 <- ctree(default_next ~ max_DPD_upto + ratio_passed_installments + last_payment + payments_per_installments,
                             data = df80, 
                             controls = ctree_control(mincriterion = 0.99, minsplit = 250))
plot(pd_dt_bucket2)

# Get stats
df80$cluster <- where(pd_dt_bucket2)
...
score_train <- predict(pd_dt_bucket2, df80)
score_test <- predict(pd_dt_bucket2, df20)
...
score_all <- predict(pd_dt_bucket2, df)
...
splitted_names(pd_dt_bucket2)

# Output model 
setwd("C:\\Projects\\PD_model\\models\\")
save(pd_dt_bucket2, file = "PD_Bucket2.RData")
