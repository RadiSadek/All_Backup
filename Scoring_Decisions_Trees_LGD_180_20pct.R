...
df$return_rate <- ifelse(df$return_rate>0.2,1,0)
df$return_rate180 <- ifelse(df$return_rate180>1, 1, ifelse(df$return_rate180<0,0,df$return_rate180))
df$return_rate180 <- ifelse(df$return_rate180>0.2,1,0)
df <- subset(df,df$EAD180>10)
...
list_var_model <-  c("return_rate","gender", "flag_paid_installments","last_payment_before_def","ratio_last_amount_paid",
                     ...
                     "payment_time_diff180", "maturity","total_income","ratio_paid_installments", "ratio_passed_installments",
                     "refinance_previous","default_previous","default_cum","refinance_cum","return_rate180","paid_time180","return_rate180")
...
split <- sample.split(df_select$return_rate180, SplitRatio = 0.8)
...
decision_tree_model <- ctree(return_rate180 ~ education + ratio_passed_installments + ratio_last_amount_paid + 
                             max_delay + gender + age + ratio_installment_income + 
                             days_diff_last_credit + experience_employer + period + payment_time_diff180 + 
                             credits_cum, 
                             data = df80, 
...
ROCRpred <- ROCR::prediction (score_train, df80$return_rate180)
...
ROCRpred <- ROCR::prediction (score_test, df20$return_rate180)
ROCRperf <- performance (ROCRpred, "tpr", "fpr")
plot(ROCRperf,col='blue', lwd=5)
par(new=TRUE)
plot(y, col='green',axes=F,xlab='',ylab='',lwd=1)
...
bins <- c(0,0.05,0.1,0.15,0.2,0.25,0.3,0.35,0.4,0.45,0.5,0.6,1)
...
df_ks <- data.frame(t(table(df_train$return_rate180,df_train$Score))[,1])
names(df_ks)[1] <- "True_Goods_Train"
df_ks$True_Bads_Train <-t(table(df_train$return_rate180,df_train$Score))[,2]
df_ks$True_Goods_Test <- t(table(df_test$return_rate180,df_test$Score))[,1]
df_ks$True_Bads_Test <- t(table(df_test$return_rate180,df_test$Score))[,2]
...
accuracy_table <- table(df80$return_rate,predict)
...
decision_tree_model_180 <- decision_tree_model
setwd("C:\\Projects\\LGD_model\\models\\20pct\\")
save(decision_tree_model_180, file = "Decision_Trees_180_20pct.rdata")
