...
# Load data to score 
credits <- read.xlsx("замяна цесия.xlsx")
credits <- as.data.frame(credits)

# Select credits to score
df <- merge(df , credits, by.x="credit_number", by.y="Номер.на.договор", TRUE, all.y=TRUE)
...
df$collected <- ifelse(df$Сума.до.края.на.кредита.с.такси<0, 0, df$RR * df$realEAD)
sum(df$collected)/sum(df$Сума.до.края.на.кредита.с.такси)*100
...
# Output table
df_output <- df[,c("credit_number","Име.на.клиент","Сума.до.края.на.кредита.с.такси","payment_time_diff300","RR")]
write.xlsx(df_output,"Cessions_LGD_scored_14_11_2018.xlsx")
