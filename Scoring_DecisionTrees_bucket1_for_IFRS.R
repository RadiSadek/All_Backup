...
df <- read.csv("buckets\\Bucket1.csv")
nrow(df)
...
pd_dt_bucket1 <- ctree(default_next ~ max_DPD_upto + ratio_passed_installments + last_payment + payments_per_installments,
                             data = df80, 
                             controls = ctree_control(mincriterion = 0.99, minsplit = 250))
plot(pd_dt_bucket1)
splitted_names(pd_dt_bucket1)

# Get stats
df80$cluster <- where(pd_dt_bucket1)
...
score_train <- predict(pd_dt_bucket1, df80)
score_test <- predict(pd_dt_bucket1, df20)
...
ROCRpred <- ROCR::prediction (as.numeric(score_test), as.numeric(df20$default_next))
ROCRperf <- performance (ROCRpred, "tpr", "fpr")
plot(ROCRperf,col='blue', lwd=5)
par(new=TRUE)
plot(y, col='green',axes=F,xlab='',ylab='',lwd=1)
...
score_all <- predict(pd_dt_bucket1, df)
...
save(pd_dt_bucket1, file = "PD_Bucket1.RData")
